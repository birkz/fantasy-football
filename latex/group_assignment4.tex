...
\node[umlclass] (LeaguePanel)
 ...
 \nodepart{four}
  \begin{tabular}{l}
   + \texttt{LeaguePanel()}\\
   + \texttt{Dimension getPreferredSize()}\\
  \end{tabular}
 };
\end{scope}
...
\node[umlclass] (MarketPanel)
 ...
 \nodepart{three}
  \begin{tabular}{l}
   ...
   -- \texttt{String player_choice}\\
   -- \texttt{String team_choice}\\ 
   -- \texttt{String pos_choice}\\
   -- \texttt{JTable jtable}\\
   -- \texttt{JScrollPane scroll}\\ 
   -- \texttt{JPanel wrapper}\\
   -- \texttt{String text}\\
   -- \texttt{List<PlayerInterface> results}\\ 
  \end{tabular}
 \nodepart{four}
  \begin{tabular}{l}
   + \texttt{MarketPanel(JScrollPane scroll, int value)}\\
   + \texttt{JComboBox<String> addComboBox(List<String> choices, String flag)}\\
   + \texttt{static LeagueMock getLeague()}\\
   + \texttt{void refreshJTable()}\\
   + \texttt{JTable getJTable(String player_searched, String team_searched, String pos_searched)}\\
   + \texttt{Object[][] getTableData()}\\
   + \texttt{String positionToString(Position pos)}\\
   + \texttt{Dimension getPreferredSize()}\\
  \end{tabular}
 };
\end{scope}
...
\node[umlclass] (RosterPanel)
 ...
 \nodepart{three}
  \begin{tabular}{l}
   -- \texttt{Roster roster}\\
   -- \texttt{JLabel num_players}\\
   -- \texttt{final Integer IS_ON_FIELD_COLUMN}\\ 
  \end{tabular}
 \nodepart{four}
  \begin{tabular}{l}
   + \texttt{RosterPanel()}\\
...
%% ==========================================
\begin{scope}[xshift=0cm,yshift=0cm]
\node[umlclass] (GraphData)
 {\textbf{\large \textit{GraphData}}
 \nodepart{two}
  {\footnotesize
  \begin{tabular}{l} 
   Desc.
...
\node[umlclass] (ButtonColumn)
 ...
 \nodepart{four}
  \begin{tabular}{l}
   ...
   + \texttt{Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column)}\\
   + \texttt{Object getCellEditorValue()}\\
   + \texttt{Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)}\\
   + \texttt{void actionPerformed(ActionEvent e)}\\
...
\node[umlclass] (CustomButton)
 ...
 \nodepart{four}
  \begin{tabular}{l}
   ...
   + \texttt{void setHoverBackgroundColor(Color hoverBackgroundColor)}\\
   + \texttt{Color getHoverFontColor()}\\
   + \texttt{void setHoverFontColor(Color hoverFontColor)}\\
   + \texttt{Color getPressedBackgroundColor()}\\
   + \texttt{void setPressedBackgroundColor(Color pressedBackgroundColor)}\\
   + \texttt{void paintComponent(Graphics g)}\\
   + \texttt{Dimension getPreferredSize()}\\
  \end{tabular}
 };
